...
for i in range(1,20):
    ...
    for j in range(len(soup.select('ul li div div'))):
        if(j %2 == 0):
            ...
            arData.append([soup.select('ul li div div')[j].get_text() + '|'+ "local-filename1"+str(j)+str(i)+".jpg"])
            fp.write(soup.select('ul li div div')[j].get_text()+ '|'+ "local-filename1"+str(j)+str(i)+".jpg" + '\n')
            # fp.write("local-filename1"+str(j)+str(i)+".jpg"+ ' ')
        # fp.write("local-filename1"+str(j)+str(i)+".jpg")
    # end save data to text
    for j in range (len(arData)):
        print(arData[j][0].split('|'))
        ws.append(arData[j][0].split('|')) 
    fp.write('\n\n')
wb.save("sample.xlsx")
fp.close()
